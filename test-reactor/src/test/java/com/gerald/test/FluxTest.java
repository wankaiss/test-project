...
public class FluxTest {
  ...
  public void testFluxWithObject() {
    ...
    Mono<Map<Integer, List<String>>> mapMono = fluxJust.collectMap(
            SimpleObject::getCount,
            SimpleObject::getItems,
            HashMap::new).map(
            map -> {
              Map<Integer, List<String>> res = new HashMap<>();
              Integer maxKey = map.keySet().stream().max((Comparator.comparingInt(Integer::intValue))).get();
              List<String> value = map.values().stream().flatMap(List::stream).collect(Collectors.toList());
              res.put(maxKey, value);
              return res;
            })
            .doOnNext(System.out::println);

//    Flux<String> flux = mapMono.flux().flatMap(map -> Flux.fromIterable(map.values())).flatMap(list -> Flux.fromIterable(list));

    StepVerifier.create(mapMono).expectNext((Map<Integer, List<String>>) new HashMap<Integer, List<String>>().put(20, asList("test"))).verifyComplete();
  }
